...
class RKD(nn.Module):
	...
	def __init__(self):
		...
		self.w_dist = CFG.w_dist
		self.w_angle = CFG.w_angle
		self.alpha = CFG.lamda_kd
		self.hard_loss = nn.CrossEntropyLoss()  # task loss

	def forward(self, feat_s, feat_t, students_preds, hard_label):
		distillation_loss = self.w_dist * self.rkd_dist(feat_s, feat_t) + \
			   self.w_angle * self.rkd_angle(feat_s, feat_t)
		distillation_loss = 0.1 * distillation_loss
